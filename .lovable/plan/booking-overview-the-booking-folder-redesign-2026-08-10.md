...
3. **Booking summary strip** — the closing element of the PRIMARY overview composition, directly after Current Action + What Happens Next + Booking Details / Need Help: one horizontal ivory raised strip split into four equal cells with thin dividers (guests, rooms, stay dates, documents uploaded), each with a medallion icon and a `View details →` link to the existing panel/tab. It is never placed above the primary content, near the tabs, or after all secondary content. The KPI strip that currently sits above the tabs is removed from that position.
4. **More booking information** — below the summary strip, the page continues downward as a natural extension of the same folder: compact ivory-on-stone sections (rooms, dining, services, special requests, recent activity, documents, notes, deadlines, rooming list progress) with calm spacing and no SaaS accordion chrome. Disclosure is used only where a section is long; everything keeps its current functionality and edit panels.

Final order: hero → folder tabs → Current Action → What Happens Next + Booking Details / Need Help → Booking Summary Strip → More booking information → additional secondary content.
